...
import { cellCoords, cellProps, flagsAmountProps, settingsProps } from "./types";
import { findFrontCell, openCell, updateStatus } from "./utils";
        ...
        <div
          className={clsx(
            `cell relative inline-block w-[32px] h-[32px] bg-gradient-to-br from-gray-50 to-gray-400 border-4 border-slate-500 border-t-slate-200 border-l-slate-200 border-solid text-center text-base font-bold select-none`,
            {
              ...
              "cursor-default after:block": cell.status.find(i => i.isActive)?.status !== 'none',
              "after:bg-[url('/flag.png')]": cell.status.find(i => i.isActive)?.status === 'flag',
              "after:bg-[url('/unknown.png')]": cell.status.find(i => i.isActive)?.status === 'unknown',
          ...
          onContextMenu={(e) => {if (!cell.opened) return updateStatus(e, cell, settings, flagsCount, setFlagsCount, setGameState)}}
        >
